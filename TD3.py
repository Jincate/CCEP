...
class Critic(nn.Module):
	def __init__(self, state_dim, action_dim):
		...
		self.l6 = nn.Linear(256 + action_dim, 1)
	...
	def train(self, replay_buffer, batch_size=256):
		...
		if self.total_it % self.policy_freq == 0:
			...
			for param, target_param in zip(self.actor.parameters(), self.actor_target.parameters()):
				target_param.data.copy_(self.tau * param.data + (1 - self.tau) * target_param.data)
		self.logger.add_scalar("currentQ1", current_Q1.max(), self.total_it)
		self.logger.add_scalar("currentQ2", current_Q2.max(), self.total_it)
		self.logger.add_scalar("targetQ", target_Q.max(), self.total_it)
