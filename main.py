...
def eval_policy(policy, env_name, seed, eval_episodes=10):
	...
	for _ in range(eval_episodes):
		...
		while not done:
			action = policy.select_action(np.array(state), context)
...
if __name__ == "__main__":
	...
	replay_buffer = utils.ReplayBuffer(state_dim, action_dim, args.num_skills)
	# Evaluate untrained policy
	evaluations = [eval_policy(policy, args.env, args.seed, args.num_skills)]
	state, done = env.reset(), False
	episode_reward = 0
	episode_timesteps = 0
	episode_num = 0
	epsilon = 0.999
	initial_index = np.random.randint(0, high=args.num_skills)
	initial_context = np.zeros(args.num_skills)
	initial_context[initial_index] = 1
	zero_context = np.zeros(args.num_skills)
	zero_context[0] = 1
	...
	for t in range(int(args.max_timesteps)):
		...
		if t < args.start_timesteps:
			...
			action = (
				policy.select_action(np.array(state), context)
